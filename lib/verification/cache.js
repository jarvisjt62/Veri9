...
const ENGINE_VERSION = 22; // Round 30f — GS1_COUNTRY_MISMATCH severity now gated on countryEvidenceCount (sources that actually supplied a country claim) instead of richSourcesCount (any source with product data, including country-blind UPCitemdb). Fixes non-deterministic COUNTERFEIT verdicts that flipped run-to-run purely based on whether UPCitemdb timed out (e.g. Optimum Nutrition 748927028690, Dymatize Iso100 705016353156). Also removed the openFoodFactsRegional double-count fix's debug console.log.
