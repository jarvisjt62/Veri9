...
function detectCounterfeitSignals({
  ...
  if (gs1Info && gs1Info.country && gs1Info.country !== 'Unknown') {
    ...
    if (!anyMatch && nonNoiseClaimed.length > 0) {
      ...
      } else if (richSourcesCount <= 1) {
        // Round 30: With 0-1 rich sources, a country mismatch is almost always
        // a data quality artifact (one crowd-sourced DB says "United States" while
        // the GS1 prefix says "Colombia" because the brand registered there).
        // Downgrade from HIGH to MEDIUM and reduce score — this should NOT
        // trigger COUNTERFEIT on its own when the evidence is thin.
        signals.push({
