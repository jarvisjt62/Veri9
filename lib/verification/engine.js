...
async function _verifyProductInner(barcode) {
  ...
  const [
    ...
    // Round 28b — Tier 1 free sources
    openProductsFacts.lookupByBarcode(barcode),
    openPetFoodFacts.lookupByBarcode(barcode),
    wikidata.lookupByBarcode(barcode),
  ]);
  ...
  // Determine product type EARLY so the trust score can use it to apply
  // the right scoring weights (e.g. don't penalize an electronics product
  // for not being in the FDA drug database).
  ...
  const trustScore = calculateTrustScore(effectiveOffData, fdaData, barcodeData, obfData, foundSources, upcData, libData, datakickData, gs1Info, productTypeEarly, amazonData, barcodeSpiderData, eanDbData, eanData);
  ...
  const brandForRecall = effectiveOffData.brand || upcData.brand || datakickData.brand || (wikidataData && wikidataData.brand) || (opfData && opfData.brand);
  // Round 28b — also resolve product name early so recall matchers can use it
  const productNameForRecall = effectiveOffData.name || upcData.name || datakickData.name || (opfData && opfData.name) || (opetfData && opetfData.name) || (wikidataData && wikidataData.name) || '';
  if (brandForRecall && brandForRecall !== 'Unknown') {
    const [foodRecallsRes, drugRecallsRes, hcRecallsRes] = await Promise.allSettled([
      ...
      healthCanadaRecalls.lookupByBrand(brandForRecall, productNameForRecall),
    ]);
  ...
  // NOTE: We computed productTypeEarly above for trust scoring; reuse it here.
  const productType = productTypeEarly;

  // Build unified product info
  const productInfo = buildProductInfo(effectiveOffData, fdaData, barcodeData, obfData, upcData, libData, datakickData, gs1Info, productType, amazonData, barcodeSpiderData, eanDbData, eanData, opfData, opetfData, wikidataData);
 ...
 */
function calculateTrustScore(offData, fdaData, barcodeData, obfData, foundSources, upcData, libData, datakickData, gs1Info, productType, amazonData, barcodeSpiderData, eanDbData, eanSearchData) {
  let score = 0;
  let maxScore = 0;
  ...
  const richSources = (foundSources || []).filter(s => s && s.name && s.name !== 'Unknown' && String(s.name).trim().length > 0);
  const richCount = richSources.length;
  const totalCount = (foundSources || []).length;
 ...
 */
function buildProductInfo(offData, fdaData, barcodeData, obfData, upcData, libData, datakickData, gs1Info, productType, amazonData, barcodeSpiderDataRef, eanDbDataRef, eanSearchData, opfData, opetfData, wikidataData) {
  const info = {
    name: null,
    brand: null,
    manufacturer: null,
