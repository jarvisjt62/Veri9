...
function calculateTrustScore(offData, fdaData, barcodeData, obfData, foundSources, upcData, libData, datakickData, gs1Info, productType, amazonData, barcodeSpiderData, eanDbData, eanSearchData) {
  ...
  // Amazon PA-API exact match, Barcode Spider, EAN-DB — these are real
  // catalogs with images and rich metadata; an exact hit is a strong
  // authenticity signal even if Open Food Facts / FDA missed the product.
  maxScore += 10;
  ...
  score += Math.min(10, enrichmentScore);
