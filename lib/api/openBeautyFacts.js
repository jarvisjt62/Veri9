 ...
 * Free, open-source database for cosmetics and personal care products
 * Sister project to Open Food Facts — no API key required
 */

const BASE_URL = 'https://world.openbeautyfacts.org/api/v2';

/**
 * Look up a cosmetic/beauty product by barcode
...
async function lookupByBarcode(barcode) {
  try {
    ...
    return { found: false, source: 'Open Beauty Facts', barcode };
  } catch (error) {
    console.error('Open Beauty Facts lookup error:', error.message);
    return { found: false, source: 'Open Beauty Facts', barcode, error: error.message };
  }
}

/**
 * Search for beauty/cosmetic products by name
...
async function searchByName(query, pageSize = 20) {
  try {
    ...
    console.error('Open Beauty Facts search error:', error.message);
    return [];
  }
}
