...
function getGS1Info(barcode) {
  ...
  const len = cleanBarcode.length;
  const barcodeType = getBarcodeType(cleanBarcode);

  // CRITICAL FIX: UPC-A (12-digit) barcodes are US/Canada native. They do NOT
  // follow the GS1 country-prefix scheme — the first digit is a "number system
  // character" (0-1, 6-9 = manufacturer-assigned US/Canada; 2 = in-store;
  // 3 = drugs (NDC); 4 = loyalty). Treating a UPC-A like an EAN-13 gives
  // completely wrong countries (e.g. Mrs Dash 850062253469 → "Cuba").
  //
  // When we have a 12-digit UPC-A, we treat it as US/Canada unless the number
  // system digit says otherwise. We do NOT return a misleading "Cuba/South
  // Africa/etc" country based on the first 3 digits.
  if (len === 12) {
    const nsChar = cleanBarcode[0];
    let country = 'United States / Canada';
    if (nsChar === '2') country = 'In-store use (variable weight / restricted)';
    else if (nsChar === '3') country = 'United States (drug / pharmaceutical NDC)';
    else if (nsChar === '4') country = 'In-store use (loyalty / restricted)';
    else if (nsChar === '5') country = 'United States (coupon)';
    return {
      country,
      prefix: nsChar,
      barcodeType,
  ...
  // EAN-13 barcodes with leading "0" are really UPC-A with a zero pad — treat
  // them the same way (the real first digit is the number system character).
  if (len === 13 && cleanBarcode[0] === '0') {
    const nsChar = cleanBarcode[1];
    let country = 'United States / Canada';
    if (nsChar === '2') country = 'In-store use (variable weight / restricted)';
    else if (nsChar === '3') country = 'United States (drug / pharmaceutical NDC)';
    else if (nsChar === '4') country = 'In-store use (loyalty / restricted)';
  ...
  // Try 3-digit prefix first (true EAN-13 country prefix)
  ...
  // Try 2-digit prefix (legacy)
  const prefix2 = cleanBarcode.substring(0, 2);
  const twoDigitMap = {
    '30': 'France', '40': 'Germany', '45': 'Japan',
