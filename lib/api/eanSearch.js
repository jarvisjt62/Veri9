...
/**
 * EAN-Search.org barcode lookup
 * Free tier available — use EAN_SEARCH_TOKEN env var for authenticated requests.
 * Docs: https://www.ean-search.org/perl/api.pl
 * @param {string} barcode
 */
async function lookupOpenEAN(barcode) {
  try {
    const token = process.env.EAN_SEARCH_TOKEN;
    // Build query: authenticated if token present, otherwise try unauthenticated free tier
    const params = new URLSearchParams({ q: barcode, lang: '1', format: 'json' });
    if (token) params.set('token', token);
    const url = `https://ean-search.org/perl/api.pl?${params.toString()}`;
    const response = await fetch(url, {
      headers: {
