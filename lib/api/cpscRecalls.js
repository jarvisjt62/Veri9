...
async function lookupByBarcode(barcode) {
  try {
    ...
    if (Array.isArray(data) && data.length > 0) {
      const recall = data[0];
      // Guard: CPSC API sometimes returns an error object with RecallID=0
      // and a "Title" containing "Error retrieving Recalls". This is NOT a real recall.
      if (!recall.RecallID || recall.RecallID === 0 || (recall.Title && recall.Title.includes('Error retrieving'))) {
